...
int uzfs_zpool_create(int argc, char **argv)
{
	int err = 0;
	char *zpool = argv[1];
	char *path = argv[2];
	...
	if (err) {
		...
		printf("created zpool: %s, path: %s\n", zpool, path);
	}
	return err;
}

int uzfs_zpool_destroy(int argc, char **argv)
{
	int err = 0;
	char *zpool = argv[1];
	...
	if (err) {
		...
		printf("Destroyed zpool: %s\n", zpool);
	}
	return err;
}

int uzfs_zpool_set(int argc, char **argv)
{
	char *zpool = argv[1];
...
int uzfs_zpool_get(int argc, char **argv)
{
	int err = 0;
	char *zpool = argv[1];
	char *prop_name = argv[2];
...
int uzfs_dataset_create(int argc, char **argv)
{
	int err = 0;
	char *dsname = argv[1];
...
int uzfs_dataset_destroy(int argc, char **argv)
{
	char *dsname = argv[1];
	...
	printf("Destroyed dataset: %s\n", dsname);
	return 0;
}


int uzfs_object_create(int argc, char **argv)
{
	int err = 0;
	char *dsname = argv[1];
	...
	if (err) {
		...
		printf("created object: %ld on dataset %s\n", obj, dsname);
	}
...
int uzfs_object_delete(int argc, char **argv)
{
	int err = 0;
	char *dsname = argv[1];
...
int uzfs_object_claim(int argc, char **argv)
{
	int err = 0;
	char *dsname = argv[1];
	uint64_t obj = atoi(argv[2]);
	...
	if (err) {
		...
		printf("claimed object: %ld on dataset %s\n", obj, dsname);
	}
...
int uzfs_object_stat(int argc, char **argv)
{
	int err = 0;
	char *dsname = argv[1];
	uint64_t obj = atoi(argv[2]);
...
int uzfs_object_list(int argc, char **argv)
{
	char *dsname = argv[1];
	...
	printf("Listed object: %s\n", dsname);

	libuzfs_dataset_close(dhp);
	return 0;
}

int uzfs_object_read(int argc, char **argv)
{
	int err = 0;
	char *dsname = argv[1];
	uint64_t obj = atoi(argv[2]);
	...
	if (err) {
		...
		printf("Read object: %s:%ld, offset: %d, size: %d\n", dsname, obj, offset, size);
		printf("%s\n", buf);
	}
...
uzfs_object_write(int argc, char **argv)
{
	...
	if (err) {
		...
		printf("Write object: %s:%ld, offset: %d, size: %d\n", dsname, obj, offset, size);
		printf("%s\n", buf);
	}
