...
static uint64_t
libuzfs_random(uint64_t range)
{
	uint64_t r;

	ASSERT3S(libuzfs_fd_rand, >=, 0);

	if (range == 0)
		return (0);

	if (read(libuzfs_fd_rand, &r, sizeof (r)) != sizeof (r))
...
{
	return (SPA_MINBLOCKSHIFT + libuzfs_random(5));
}

static nvlist_t *
make_vdev_file(const char *path, char *aux, const char *pool, size_t size, uint64_t ashift)
{
	char *pathbuf;
	nvlist_t *file;
...
static nvlist_t *
make_vdev_root(const char *path, char *aux, const char *pool, size_t size, uint64_t ashift,
...
void libuzfs_init()
{
	...
	libuzfs_fd_rand = open(random_path, O_RDONLY);
	ASSERT3S(libuzfs_fd_rand, >=, 0);
...
{
	kernel_fini();
	(void) close(libuzfs_fd_rand);
}

// for now, only support one device per pool
int libuzfs_zpool_create(const char *zpool, const char *path, nvlist_t *props, nvlist_t *fsprops)
{
	nvlist_t* nvroot;
	int err = 0;

	nvroot = make_vdev_root(path, NULL, zpool, 1, 0, NULL, 0, 0, 1);
...
int libuzfs_zpool_destroy(const char *zpool)
{
	return spa_destroy(zpool);
}

libuzfs_zpool_handle_t* libuzfs_zpool_open(const char *zpool)
{
	int err = 0;
	spa_t *spa;
	...
	libuzfs_zpool_handle_t *zhp = umem_alloc(sizeof(libuzfs_zpool_handle_t), UMEM_NOFAIL);
	zhp->spa = spa;
	(void) strlcpy(zhp->zpool_name, zpool, sizeof (zhp->zpool_name));
	return zhp;
}

void libuzfs_zpool_close(libuzfs_zpool_handle_t *zhp)
{
...
void libuzfs_zpool_prop_set(libuzfs_zpool_handle_t *zhp, zpool_prop_t prop, uint64_t value)
{
	libuzfs_spa_prop_set_uint64(zhp->spa, prop, value);
}


int libuzfs_zpool_prop_get(libuzfs_zpool_handle_t *zhp, zpool_prop_t prop, uint64_t *value)
{
	nvlist_t *props = NULL;
	...
	nvlist_t *propval;
	if (nvlist_lookup_nvlist(props, zpool_prop_to_name(prop), &propval) != 0) {
		return -1;
	}
...
libuzfs_objset_create_cb(objset_t *os, void *arg, cred_t *cr, dmu_tx_t *tx)
{
	 ...
	 */
	VERIFY0(zap_create_claim(os, LIBUZFS_DIROBJ, DMU_OT_ZAP_OTHER, DMU_OT_NONE, 0, tx));
}
...
libuzfs_dataset_create(const char *dsname)
{
	...
	err = dmu_objset_create(dsname, DMU_OST_ZFS, 0, NULL, libuzfs_objset_create_cb, NULL);
	if (err) return (err);

	return (libuzfs_dsl_prop_set_uint64(dsname, ZFS_PROP_SYNC, ZFS_SYNC_ALWAYS, B_FALSE));
}
...
libuzfs_objset_destroy_cb(const char *name, void *arg)
{
	 ...
	 */
	VERIFY0(libuzfs_dmu_objset_own(name, DMU_OST_ZFS, B_TRUE, B_TRUE, FTAG, &os));
	error = dmu_object_info(os, LIBUZFS_DIROBJ, &doi);
	if (error != ENOENT) {
...
{
	(void) dmu_objset_find(dsname, libuzfs_objset_destroy_cb, NULL, DS_FIND_SNAPSHOTS | DS_FIND_CHILDREN);
}
...
libuzfs_dataset_handle_t * libuzfs_dataset_open(const char *dsname)
{
	...
	VERIFY0(libuzfs_dmu_objset_own(dsname, DMU_OST_ZFS, B_FALSE, B_TRUE, dhp, &os));
...
void libuzfs_dataset_close(libuzfs_dataset_handle_t *dhp)
{
	zil_close(dhp->zilog);
...
int libuzfs_object_stat(libuzfs_dataset_handle_t *dhp, uint64_t obj, dmu_object_info_t *doi)
{
	dmu_buf_t *db;
	objset_t *os = dhp->os;
...
int libuzfs_object_create(libuzfs_dataset_handle_t *dhp, uint64_t *obj)
{
	int error = 0;
	dmu_tx_t *tx;
	dmu_buf_t *db;
...
int libuzfs_object_delete(libuzfs_dataset_handle_t *dhp, uint64_t obj)
{
	int error = 0;
	dmu_tx_t *tx;
	objset_t *os = dhp->os;
...
int libuzfs_object_claim(libuzfs_dataset_handle_t* dhp, uint64_t obj)
{
	int error = 0;
	dmu_tx_t *tx;
	dmu_buf_t *db;
...
int libuzfs_object_list(libuzfs_dataset_handle_t* dhp)
{
	int err;
	int i = 0;
	uint64_t obj;
...
int
libuzfs_object_write(libuzfs_dataset_handle_t* dhp, uint64_t obj, int offset, int size, const char *buf)
{
	int error = 0;
	objset_t *os = dhp->os;
	dmu_tx_t *tx;
...
int
libuzfs_object_read(libuzfs_dataset_handle_t* dhp, uint64_t obj, int offset, int size, char *buf)
{
	int err = 0;
	objset_t *os = dhp->os;
	dmu_object_info_t doi;
